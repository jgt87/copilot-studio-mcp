...
| 25 Long-running tools block the client (found live 2026-09-09) | open | `cs_pull_solution` does export (twice with the default `Both`), unpack, create-settings and a `pac copilot clone` per agent inside one MCP call. The server allows an hour per export; the MCP client caps the call long before that and the work is orphaned. Found on a real tenant during Phase G. Workaround: `packagetype: "Unmanaged"` + `cloneAgents: false`, or run the three pac commands in a terminal. Fix: a background job mode, as `cs_login` already does with `status: pending` |
